...
import { Ic, Logo } from "@/components/recon/ReconUI";
...
export const ReconSidebar = ({ role, active, setActive, user }: SidebarProps) => {
  const nav = NAV[role] || NAV.owner;
  const roleInfo = ROLES[role];
  return (
    <div style={{ width: 230, minWidth: 230, background: T.surface, borderRight: `1px solid ${T.border}`, display: "flex", flexDirection: "column", height: "100vh", position: "sticky", top: 0, overflowY: "auto" }}>
      <div style={{ padding: "18px 18px 14px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 10 }}>
        <Logo size={34}/>
        <div>
...
export const TopBar = ({ pageTitle, role, onNewJob, onRoleChange, onSignOut }: TopBarProps) => (
  <div style={{ background: T.surface, borderBottom: `1px solid ${T.border}`, padding: "0 28px", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10 }}>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <select value={role} onChange={e => onRoleChange(e.target.value)} style={{ background: T.surfaceHigh, border: `1px solid ${T.border}`, borderRadius: 7, padding: "5px 10px", color: T.muted, fontSize: 11, fontFamily: "'DM Sans',sans-serif", cursor: "pointer", outline: "none" }}>
        {Object.entries(ROLES).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
      </select>
      {ROLES[role]?.canViewAllJobs !== false && <Btn v="primary" sz="sm" icon="plus" onClick={onNewJob}>New Job</Btn>}
