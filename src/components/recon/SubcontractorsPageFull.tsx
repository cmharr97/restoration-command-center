...
export const SubcontractorsPageFull = () => {
  ...
  const tradeColors: Record<string, string> = {
    Roofing: T.redBright, Drywall: T.blueBright, Flooring: T.tealBright,
    Cabinets: T.purpleBright, Painting: T.yellowBright, Framing: T.orange,
    Plumbing: T.blueBright, Electrical: T.yellowBright, HVAC: T.greenBright,
  };
    ...
    <div style={{ padding: "0 0 40px" }}>
      <div style={{ padding: "24px 28px 0", display: "flex", justifyContent: "space-between", marginBottom: 18 }}>
        <div>
          ...
          <p style={{ margin: "3px 0 0", color: T.muted, fontSize: 13 }}>Manage trade partners — {subs.length} subcontractor{subs.length !== 1 ? "s" : ""}</p>
        </div>
      ...
      <div style={{ padding: "0 28px" }}>
        ...
        {subs.length === 0 ? (
          <Card style={{ textAlign: "center", padding: 48 }}>
            ...
            <div style={{ fontSize: 13, color: T.muted, marginTop: 6 }}>Add your drywall, flooring, paint, plumbing, and roofing subcontractors.</div>
          </Card>
        ) : (
            ...
            {filtered.map((sub: any) => (
              <Card key={sub.id}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  ...
                  <Badge color={tradeColors[sub.trade] ? (sub.trade === "Roofing" ? "red" : sub.trade === "Drywall" ? "blue" : "teal") : "gray"} small>{sub.trade}</Badge>
                </div>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                  <Badge color={sub.status === "active" ? "green" : "gray"} small dot>{sub.status}</Badge>
                </div>
              </Card>
            ))}
          </div>
